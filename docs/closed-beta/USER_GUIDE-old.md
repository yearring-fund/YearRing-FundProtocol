...
参与需要通过 allowlist 审核。Closed beta 的目的是在受控条件下验证协议的链上行为，不是公开发布。

---

## 2. 合约状态

所有合约已部署在 Base 主网，并通过了内部排练测试。
...
存款资产为 USDC。用户存入 USDC 后，协议将部分资产通过 Aave V3 出借以产生利息。
...
实际收益取决于 Aave V3 的实时借贷利率，该利率随市场供需变化。利率可能升高，也可能降低，也可能接近零。

协议收取管理费：**4 bps/月（约 0.48%/年）**，从收益中扣除。管理费计入 Treasury，不直接从本金扣除，但会影响净收益。
...
**YRPTS 不是已确认的原生代币，也不代表任何未来空投或经济权益的承诺。**
...
- 合约未经外部审计，存在未知漏洞风险
- 收益不固定，可能为零
- 本金无安全保障：链上协议存在智能合约风险、策略风险
- Aave V3 本身也存在流动性风险和清算机制风险
- 提前退出将损失全部 Points
...
*YearRing Fund Protocol — Closed Beta 内部文档*  
*SiLugang — 2026-05-07*
